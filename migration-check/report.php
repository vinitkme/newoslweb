    ...
    if($jsonFileType != 'json')
    {
        ...
        echo "Please upload JSON File Only";
         echo '<script type="text/javascript">
           window.location = "http://opensenselabs.com/migration-check"
      		</script>';
        die();
    }
}
...
$sql0 = "INSERT INTO website (Name,URL,Admin,Email,Phone,JSONFile,Date) VALUES ('".$websitename."','".$url."','".$admin."','".$email."','".$mobile."','".$file."','".$date."')";
...
$shareLink = "http://opensenselabs.com/migration-report/".$websitename;
...
mail('devanshu@opensenselabs.com','Score Checked by '.$websitename,$mailmessage);
header("Location: ".$shareLink);
...
<div class="row header-section columns">
    ...
    <nav class="top-bar important-class" data-topbar="">
      ...
      <section class="top-bar-section float-right">
        <ul class="right menu">
        <form action="report-updated.php" method="post">
          Price($/hr): <input type="number" name="priceph" value="35">
          <input type="hidden" name="email" value="<?php echo $email; ?>">
          <input type="hidden" name="websitename" value="<?php echo $websitename; ?>">
        ...
        <div class="migration_details_block_new columns">
          ...
          <div class="flex-row" style="margin-top:30px">
            <ul class="menu mig-value">
              ...
              <li><span class="value-block effort"><?php echo round($man_time_min*$priceph,0)." to ".round($man_time_max*$priceph,0)." $"; ?></span><span class="value-label">Estimated Cost</span></li>
            </ul>
          </div>
        </div>
        </div>
...
<table id="myTable">
  <thead><tr><th>Select</th><th>Type</th><th>Count</th><th>D8 Version Available</th><th>Additional Time Required for Porting/Migrating</th><th>Estimated Cost($)</th></tr></thead>
  <tbody>
  <tr>
...
<ul class="menu">
<li><h4><i class="fa fa-clock-o" aria-hidden="true"></i> Time : <span><span id='total_time_min'><?php echo round($man_time_min+0,0);  ?></span> to <span id='total_time_max'><?php echo round($man_time_max+1,0); ?></span></span></h4></li>
...
<div class="estimation-block panel">
  <ul class="menu">
<li><h4><i class="fa fa-clock-o" aria-hidden="true"></i> Time : <span><span id='time_min_checkbox'><?php echo round($modules_features_min+$themes_info['min_hour'],0)  ?></span> to <span id='time_max_checkbox'><?php echo round($modules_features_max+$themes_info['max_hour'],0) ?></span></span></h4>
...
<div>
<p>Time : <?php echo round($man_days_min,0)." to ".round($man_days_max,0)." Days"; ?></p><br>
<p>Cost : USD <?php echo round($man_time_min*$priceph,0)." to ".round($man_time_max*$priceph,0)." $"; ?></p>
</div>
<div class="row flex-row top40 columns">
<div class="report-table email-mig-wrap">
<div class="medium-7 columns email-mig">
<h3>Share Report</h3>
<input type="text" id="copyTarget" value="<?php echo $shareLink; ?>"> <button id="copyButton">Copy</button>

<!--<p><a href="<?php echo $shareLink; ?>">Report</a></p>-->
...
<form action="sendmail.php" method="post" class="all-set">
 ...
 <input type="hidden" name="mailmessage" value="<?php echo $mailmessage; ?>">
